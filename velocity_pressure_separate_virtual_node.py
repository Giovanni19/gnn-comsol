...
from training_separate_virtual_node import train_network
...
# Velocity target remains in physical units
Y_train_velocity = Y_train[:, :, 0:2]
Y_val_velocity   = Y_val[:, :, 0:2]
Y_test_velocity  = Y_test[:, :, 0:2]

# Pressure target is normalized
Y_train_pressure = Y_train[:, :, 2:3]
Y_val_pressure   = Y_val[:, :, 2:3]
Y_test_pressure  = Y_test[:, :, 2:3]

Y_train_pressure = (
    Y_train_pressure - pressure_mean
) 

Y_val_pressure = (
    Y_val_pressure - pressure_mean
) 

Y_test_pressure = (
    Y_test_pressure - pressure_mean
) 
...
Y_train_velocity = torch.tensor(
    Y_train_velocity,
    dtype=torch.float32
)

Y_val_velocity = torch.tensor(
    Y_val_velocity,
    dtype=torch.float32
)

Y_test_velocity = torch.tensor(
    Y_test_velocity,
    dtype=torch.float32
)
...
Y_train_pressure = torch.tensor(
    Y_train_pressure,
    dtype=torch.float32
)

Y_val_pressure = torch.tensor(
    Y_val_pressure,
    dtype=torch.float32
)

Y_test_pressure = torch.tensor(
    Y_test_pressure,
    dtype=torch.float32
)
...
class GCNVirtualNodeNet(nn.Module):
    ...
    def forward(self, graph):
        ...
        # During training/validation/test graph is a Batch.
        # During inference graph can be a single Data object.
        if hasattr(graph, "batch") and graph.batch is not None:
            ...
            num_graphs = 1
        # One virtual node for each graph in batch
        ...
        # Virtual node associated with each physical node
        node_virtual = virtual_indices[batch]

        physical_nodes = torch.arange(
            num_nodes,
            device=x.device
        )
        ...
        # -----------------------------------------
        # Remove virtual nodes from output
        # -----------------------------------------
...
pressure_net = GCNVirtualNodeNet(
    num_in=12,
    num_out=1,
    num_neurons=num_neurons,
    num_layers=num_layers,
